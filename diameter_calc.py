...
# Найти перпендикулярное направление
...
# Билинейная интерполяция для поиска значений в нецелых позициях
...
# Кластеризация K-средних точек вдоль нормали
def cluster(Z):
    z = Z.reshape((-1,1))
    z = np.float32(Z)
...
# Выбор кластера с самой низкой интенсивностью
def dia_color_min(l):
    l = l.reshape(-1, 1)
...
def diameter(img):
    ...
    # Находим  точки нормали для каждой центральной точки и строим их координаты и значения
    tempo = g.copy()
    nrp = []
    nrv = []
    for j in range(3, point.shape[0] - 3, 1):
        ...
        for i in range(1, D): #от 1 до D-1 для точек D-1 на расстоянии 1 пикселя
    ...
    # 3D f
    '''
    Этот код создает трехмерную поверхность для визуализации данных.
    Он может использоваться для отображения трехмерных данных, таких как распределение интенсивности или высоты в пространстве.
    В данном случае, код создает график "Vessel histogram map" (гистограмма сосуда), 
    где ось x представляет точки, нормальные к профилю сосуда, 
    ось y представляет точки вдоль профиля сосуда, а ось z представляет интерполированную интенсивность.
    Поверхность, построенная с помощью `plot_surface`, отображает значения интенсивности на трехмерной сетке данных.
    Цветовая шкала (`colorbar`) добавляется для наглядного отображения соответствия между цветами на поверхности и значениями интенсивности.    
    '''
    ...
    # Кластеризация точек вдоль каждой нормали
    L = np.zeros(Z.shape)
    Zc = np.zeros(Z.shape)
    ...
    # Исправление CLR, т.е. соединение ломаных линий по каждой нормали
    for i in range(L.shape[0]):
    ...
    # соединение ломаных линий вдоль осевой линии
    kernel = np.ones((5, 1), np.uint8)
    ...
    # использование симметрии для удаления нежелательных ложных срабатываний
    ...
    # Наконец, сглаживаем края профиля, соединяя пиксели, находящиеся по горизонтали на расстоянии менее 20 пикселей
    ...
    print('Длина среднего диаметра: ', np.median(diameters))
    print('Среднеквадратичное отклонение: ', np.std(diameters)/len(diameters))



# Нанесение приблизительных диаметров на изображение
    final_ann = img.copy()
