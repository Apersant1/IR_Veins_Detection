...
# Выборка минимальной дистанции между зфстью контура и выбранной точкой
...
# выбирает часть контура, ближайшую к выбранной точке
...
def select(img):
    ...
    # Поиск всех точек на контуре
    Cp = cv2.cvtColor(C, cv2.COLOR_BGR2GRAY)
    point = cv2.findNonZero(Cp)

    # Берем на вход количество частей, на которые нужно разбить контур
...
# Выбор части, если количество частей <=10
